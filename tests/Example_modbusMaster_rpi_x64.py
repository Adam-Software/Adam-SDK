...
client= ModbusClient(method = "rtu", port="/dev/ttyS0",stopbits = 1, bytesize = 8, parity = 'N', baudrate= 76800)
...
def acc_motor():
  ...
  write_registers(3, speed, 23)  # Запуск заднего мотора по адресу 23

  for i in range(62):

    speed =400 + i * 10 #плавно увеличиваем скорость моторов speed = 0-1023
    write_registers(1, speed, 22) #Запуск переднего мотора по адресу 22
    write_registers(3, speed, 22) #Запуск заднего мотора по адресу 22
...
def stop_motor():
    ...
    print(write_registers(3, 0, 23))  # Остановка заднего мотора по адресу 22
    print(write_registers(1, 0, 23))  # Остановка переднего мотора по адресу 22
